...
df_raw <- habitat_data_raw %>%
  mutate(mature = (distance_root_mature_mean_exp - distance_root_mature_obs)/(1-distance_root_mature_sd_exp),
    growth = (distance_root_growth_mean_exp - distance_root_growth_obs)/(1-distance_root_growth_sd_exp)
  ) %>%
...
```{r}
run_rf_bootstrap <- function(data, root, test_prop = 0.2, n_boot = 10, seed = 123) {
  set.seed(seed)
  ...
  for (b in seq_len(n_boot)) {
    ...
    rf <- ranger(
      formula = abundance ~ root_cm2_count + root_growth_cm2_count_24h + temp +
    ...
    train_mse <- mean((data_train$abundance - pred_train)^2)
    test_mse  <- mean((data_test$abundance - pred_test)^2)
    
    unified <- ranger.unify(rf, data_train %>% select(-abundance))
    shap_values <- treeshap(unified, data_train %>% select(-abundance))
    
    importance_df <- as.data.frame(colMeans(abs(shap_values$shaps))) %>%
      tibble::rownames_to_column("variable") %>%
      rename(importance = 2) %>%
      mutate(
        explained_variance = pmax(0, 1 - rf$prediction.error / var(data_train$abundance)),
        root_type = root,
        train_mse = train_mse,
        test_mse = test_mse,
        bootstrap = b
...
invisible(results_all_boot <- map(names(df_list), function(taxon) {
  ...
  runs <- pmap(combos, function(root_type) {
    run_rf_bootstrap(df_list[[taxon]], root_type)
  })
  ...
  shap_avg <- map_dfr(runs, ~ .x$shap) %>%
    group_by(variable, image_name, root_type) %>%
    summarise(
      shap = mean(shap),
      .groups = "drop"
    ) %>%
...
```{r}
rf_meta_all %>%
  select(taxon, root_type,n_obs, train_mse, n_train, test_mse, n_test)%>%
  ggplot(aes(taxon))+
...
rf_summary_wide <- rf_results_all %>%
  ...
  group_by(taxon, root, variable_name) %>%
  summarise(
...
rf_summary_wide_meta <- rf_summary_wide %>%
  left_join(
    rf_meta_all %>%
      select(taxon, root, n_obs, oob_error, explained_variance),
    by = c("taxon", "root")
  ) %>%
  relocate(n_obs, oob_error, explained_variance) %>%
  arrange(taxon, root) %>%
  rename(
...
shap_results_all <- shap_results_all %>%
  left_join(df_all %>% select(-abundance) %>% mutate(root_type = recode(root_type,
                    growth = "growing roots",
                    mature = "mature roots")), by = c('image_name',  'root_type'))
...
normality_table <- map_dfr(
  ...
  function(taxon_i) {
    ...
    combos <- df_taxon %>% distinct(root_type)
    map_dfr(1:nrow(combos), function(i) {
      rt <- combos$root_type[i]
      df_sub <- df_taxon %>% filter(root_type == !!rt)
      ...
      map_dfr(context_factors, function(ctx) {
        map_dfr(predictors, function(pred) {
          ...
          map_dfr(levels_ctx, function(lv) {
            ...
            tibble(taxon = taxon_i,root_type = rt,context = ctx,context_level = as.character(lv), predictor = pred,n = res$n,normal = res$normal)
          })
        })
      })
    })
...
make_heatmap_shap <- function(df, context_factor_name) {
  df %>%
    ...
    select(root_type,  predictor, taxon, p.value) %>%
    mutate(
...
shap_distrib_depth <- shap_results_all %>%
  ...
  left_join(
    results_all_shap %>% 
      ...
      select(predictor, root_type, taxon, sig) %>%
      rename(variable = predictor),
    by = c("variable", "root_type", "taxon")
  ) %>%
  drop_na() %>%
  mutate(
