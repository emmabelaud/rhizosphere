...
```{r}
output_file <- "E:/distance_data.csv"
```
...
```{r}
loc_data <- read.csv("D:/Mes Donnees/2. THÈSE/3. DIAMS/fauna_data.csv", 
                     sep = ",", na.strings = c("", "NA")) %>%
...
```{r}
write.csv(final_table, "E:/distance_habitat_data.csv", row.names = FALSE)
```
