...
```{r}
# Data import
habitat_data_raw <- read.csv("~/2. THÈSE/6. Analyse/2.Rhizosphere/4. habitat_data/distance_data_cleaned.csv") %>%
  mutate(
    date = ymd_hms(date) %>% update(minute = 0, second = 0),
    land_use = position)
...
prep_df <- function(taxon = NA) {
  ...
  df %>%
    group_by(
      ...
      land_use, crop, season, period, depth,
    ...
    mutate(
      ...
      across(c(land_use, depth, period, root_type, behavior), as.factor)
    ) %>%
    mutate(across(
...
tiff("1.violin_plot.tiff", width = 1800, height = 1700, res = 600)
panel
dev.off()
...
rf_results_all <- rf_results_all %>%
  mutate(type_variable = case_when(
    ...
    variable %in% c("land_use", "depth", "period") ~ "Contextuelles",
    TRUE ~ "Autres"
  ))
...
RF_plot
tiff("2.randomforest_plot.tiff", width=6000, height=3000, res=400)
print(RF_plot)
dev.off()
```
...
save_kable(kable_rf_results, "~/2. THÈSE/6. Analyse/2.Rhizosphere/5. analysis/S1_table_rf_results.html")
webshot("~/2. THÈSE/6. Analyse/2.Rhizosphere/5. analysis/S1_table_rf_results.html",
        file = "~/2. THÈSE/6. Analyse/2.Rhizosphere/5. analysis/S1_table_rf_results.png", zoom = 2)
...
heatmap_plot
tiff("3.heatmap_plot.tiff", width=3000, height=3500, res=400)
print(heatmap_plot)
dev.off()
```
...
tiff("S1.shap_values_distribution_plot.tiff", width=2500, height=4000, res=300)
print(distrib_plot)
dev.off()
```
