...
prep_df <- function(taxon = NA) {
  ...
  df %>%
    group_by(
      image_name, scanner, period, root_type, invertebrate_count, predators_count,
      land_use, depth,
...
```{r}
run_rf_bootstrap <- function(data, root, signe, test_prop = 0.2, n_boot = 50, seed = 123) {
  set.seed(seed)
...
invisible(results_all_boot <- map(names(df_list), function(taxon) {
  ...
  runs <- pmap(combos, function(root_type, signe) {
    run_rf_bootstrap(df_list[[taxon]], root_type, signe, n_boot = 50)
  })
  ...
  importance_avg <- map_dfr(runs, ~ .x$importance) %>%
    group_by(variable, root_type, signe) %>%
    summarise(
  ...
  shap_avg <- map_dfr(runs, ~ .x$shap) %>%
    group_by(variable, image_name, root_type, signe) %>%
    summarise(
      shap = mean(shap),
      .groups = "drop"
    ) %>%
...
```{r}
rf_meta_all %>%
  select(taxon, root_type, signe,n_obs, train_mse, n_train, test_mse, n_test)%>%
  ggplot(aes(taxon))+
  geom_point(aes(y=train_mse), color = "orange")+
  geom_point(aes(y=test_mse), color = "red")+
  facet_grid(root_type ~ signe)
```
