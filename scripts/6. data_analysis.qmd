...
#habitat_data_reframe <- habitat_data_long %>% group_by(scanner, period, Predicted.class.levelup, root_type) %>% reframe(obs = mean(obs), exp = mean(exp), z_score = mean(z_score), root_cm2_count = mean(root_cm2_count), abundance = mean(abundance))
```
...
rhizo_results <- habitat_data_reframe %>%
...
p1 <- habitat_data_reframe %>%
  ...
  geom_segment(data = rhizo_results, aes(y = Predicted.class.levelup, yend = Predicted.class.levelup, x = 0, xend = med_z),arrow = arrow(length = unit(0.20, "cm")), linewidth = 0.8, color = "#e50000") +
...
p1 + labs(title = "Rhizosphere Attraction Index",
       subtitle = "Negative Z-scores indicate active selection of the root zone")
tiff("../output/1.zscore_distribution.tiff", width = 1500, height = 1000, res = 300)
p1
invisible(dev.off())
```
...
rhizo_decay_data <- habitat_data_reframe %>%
...
t1 <- rhizo_stats_table %>%
  ...
  kbl(#caption = "Identification of the rhizosphere width through derivative analysis of animal density gradients", 
    booktabs = TRUE, align = "c", format = "html",escape = FALSE) %>%
...
save_kable(t1, "../output/2.rhizosphere_width.html")
invisible(webshot("../output/2.rhizosphere_width.html",
        file = "../output/2.rhizosphere_width.png", zoom = 2,vwidth = 1200))
```
...
p2 <- ggplot(habitat_delta, aes(x = root_cm2_count, y = delta.fitted)) +
  ...
  geom_point(aes(color = delta.fitted), size = 1, alpha=0.4) +
  ...
  # Styling
  labs(x = "Root density", y = "Fitted z-score ",color = "Index value") +
  theme_bw() +
...
#model results
#habitat_delta <- safe_augment(mecanistic_model, habitat_delta, "mech") 
...
#model results
#habitat_delta <- safe_augment(functionnal_model, habitat_delta, "func")
#gratia::draw(functionnal_model, parametric = FALSE) 
```

```{r}
tidy(mecanistic_model, parametric = FALSE) %>%
  filter(p.value < 0.05) %>%
  select(term, edf, statistic, p.value) %>%
  mutate(term = str_replace(term, "s\\(abundance\\):interaction\\(Predicted.class.levelup, root_type, drop = TRUE\\)", "abundance: "),
    term = str_replace(term, "s\\(scanner\\)", "Random Effect: Scanner"),
    term = str_replace(term, "s\\(period\\)", "Random Effect: Period"),
    p.value = scales::pvalue(p.value)
  ) %>%
  kable(
    col.names = c("Model Component", "EDF", "F-statistic", "p-value"),
    caption = "Mecanistic GAM Results",
    booktabs = TRUE, 
    digits = 3,
    align = "lccc"
  ) %>%
...
tidy(functionnal_model, parametric = FALSE) %>%
  filter(p.value < 0.05) %>%
  select(term, edf, statistic, p.value) %>%
  mutate(term = str_replace(term, "s\\(delta.resid\\):interaction\\(Predicted.class.levelup, root_type, drop = TRUE\\)", "selection strength: "),
    term = str_replace(term, "s\\(scanner\\)", "Random Effect: Scanner"),
    term = str_replace(term, "s\\(period\\)", "Random Effect: Period"),
    p.value = scales::pvalue(p.value)
  ) %>%
  kable(
    col.names = c("Model Component", "EDF", "F-statistic", "p-value"),
    caption = "Functional GAM Results",
    booktabs = TRUE, 
    digits = 3,
    align = "lccc"
  ) %>%
...
summary_table <- bind_rows(
  ...
  select(model, adj.r.squared, deviance, df, nobs)

kable(summary_table, digits = 3, col.names = c("Model", "Adj. R²", "Deviance", "df", "N Obs."),
