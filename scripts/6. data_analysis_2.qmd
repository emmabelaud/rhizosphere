...
```{r}
# Modeling the Biological Deviation from Geometry
mod_delta <- gam(
    (obs - exp) ~ Predicted.class.levelup + root_type +
    s(root_cm2_count,  by = interaction(root_type, Predicted.class.levelup,  drop = TRUE), k = 20) + 
    s(period, bs = "re") + 
    s(position, bs = "re")+
    s(depth, bs = "re"),
...
ggplot(habitat_delta, aes(x = root_cm2_count)) +
  ...
  facet_grid(root_type~Predicted.class.levelup, scale = "free") +
  labs(title = "The Biological Gap: Does Attraction scale with Density?",
       x = "Root Density",
       y = "Observed - Expected Distance (cm)") +
  theme_minimal()
```
