...
library(tidyverse)
library(data.table)
habitat_data_raw <- read.csv("~/2. THÈSE/6. Analyse/2.Rhizosphere/4. habitat_data/distance_habitat_data.csv") 
...
```{r}
probe_data_raw <- read.csv("~/2. THÈSE/3. DIAMS/sensors_data/Diams_AF1W_soil.csv") 
probe_data <- probe_data_raw %>%
  mutate(date = as.POSIXct(date, format = "%Y-%m-%d %H:%M:%S"), 
         depth=as.factor(profondeur), 
         position=as.factor(positionnement),
         hum = hum_soil, 
         temp = temp_soil) 
...
root_density <- read.csv("~/2. THÈSE/3. DIAMS/roots_data/root_pixels_count.csv")  
...
```{r}
write.csv(habitat_data, "~/2. THÈSE/6. Analyse/2.Rhizosphere/rhizosphere/output/distance_data_cleaned.csv", row.names = FALSE)
```
