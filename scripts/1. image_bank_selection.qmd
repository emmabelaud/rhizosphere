---
title: "Rhizosphère - Image bank selection"
format:
  html:
...
##Check missing data in the final dataset used in data analysis
```{r}
# Toggle this to TRUE to run the check, or FALSE to skip it
run_final_check <- TRUE

check_data_completeness <- function(expected_df, results_path) {
  # 1. Load the analyzed results
  if (!file.exists(results_path)) {
    return(message("Check skipped: Results file not found."))
  }
  
  results_df <- read.csv(results_path, sep = ",", na.strings = c("", "NA"))
  
  # 2. Get unique image names from both datasets
  expected_names <- unique(expected_df$image_name)
  analyzed_names <- unique(results_df$image_name)
  
  # 3. Identify missing images
  missing_images <- setdiff(expected_names, analyzed_names)
  
  # 4. Calculate statistics
  n_expected <- length(expected_names)
  n_analyzed <- length(analyzed_names)
  coverage_rate <- round((n_analyzed / n_expected) * 100, 2)
  
  # 5. Display Diagnosis
  cat("\n--- COMPLETENESS CHECK ---\n")
  cat("Expected images (df_final):", n_expected, "\n")
  cat("Analyzed images found:     ", n_analyzed, "\n")
  cat("Coverage rate:             ", coverage_rate, "%\n")
  
  if (length(missing_images) > 0) {
    warning(length(missing_images), " images are missing from the final database!")
    cat("First missing images:\n")
    print(head(missing_images, 5))
  } else {
    message("SUCCESS: All expected images are present in the analysis.")
  }
}

# Run the function if the toggle is TRUE
if (run_final_check) {
  check_data_completeness(df_final_filtered, "../output/distance_data_cleaned.csv")
}
```
