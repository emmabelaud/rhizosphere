...
```{r}
df_fauna <- read.csv("~/2. THÈSE/3. DIAMS/fauna_data.csv", sep = ",", na.strings = c("", "NA"))
```
...
df_complete <- df_complete %>%
  filter(orientation %in% orientation_selection)
```
...
writeLines(images_bank, "~/2. THÈSE/6. Analyse/2.Rhizosphere/2. image_bank_selection/image_bank.txt")
```

Run the Python script on Romaric (**ssh ebelaud\@147.99.15.59**) to copy the selected images:

-   `python3 /media/ebelaud/INTENSO/rhizosphere_images_bank_copy.py`

*Next step: generating habitat maps \>\> "D:\\Mes Donnees\\2. THÈSE\\6. Analyse\\Rhizosphere\\3. habitat_maps\\root_habitat_maps_generation.qmd"*
