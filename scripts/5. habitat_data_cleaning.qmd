...
habitat_data_raw <- read.csv("../output/distance_habitat_data.csv") 
...
habitat_data <- habitat_data # %>% filter(!image_name %in% outlier_images_to_filter$image_name)
```
...
habitat_data <- habitat_data %>%   
  mutate(distance_root_growth_obs = (distance_root_growth_obs/pixels_per_cm) + epsilon,         
         ...
         distance_root_growth_mean_exp = (distance_root_growth_mean_exp/pixels_per_cm) + epsilon,         
         distance_root_mature_mean_exp = (distance_root_mature_mean_exp/pixels_per_cm) + epsilon,
         distance_root_growth_sd_exp = (distance_root_growth_sd_exp/pixels_per_cm) + epsilon,         
         distance_root_mature_sd_exp = (distance_root_mature_sd_exp/pixels_per_cm) + epsilon)

#habitat_data <- habitat_data %>% mutate(across(c("distance_root_growth_sd_exp", "distance_root_mature_sd_exp"), ~ scales::rescale(.x, to = c(0, 1), na.rm = TRUE)))
```
...
relevant_taxa <- habitat_data %>%
  ...
  filter(n >= 400, !Predicted.class.levelup %in% c("unknown", "multi_taxa")) %>%
