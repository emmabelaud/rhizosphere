...
#| tbl-cap: "Summary of taxonomic occurrences and sampling effort"
...
#| fig-width: 10
#| fig-height: 8
#| fig-cap: "Final sensitivity model: aggregated response"
...
p_final <- ggplot(habitat_delta, aes(x = root_cm2_count, y = delta.fitted)) +
  ...
  scale_color_gradient2(low = "#b20000", mid = "#FFD93D", high = "grey90", midpoint = 0, guide = "none") +
  facet_grid(root_type ~ Predicted.class.levelup, scales = "free") +
  theme_bw() +
  labs(x = "Root Density (cm/cm²)", y = "Fitted Selection Intensity (z-score)")
...
get_pub_table <- function(mech_mod, func_mod) {
  extract_stats <- function(mod, label) {
    ...
    data.frame(
      ...
      Total_r2 = scales::percent(total_d2, accuracy = 0.1),
      Random_effect_r2 = scales::percent(random_d2, accuracy = 0.1),
      Marginal_r2 = scales::percent(total_d2 - random_d2, accuracy = 0.1)
    )
  }
...
unified_table <- final_metrics %>%
  ...
  select(Hypothesis, Family, Total_r2, Marginal_r2, Random_effect_r2, adj.r.squared, AIC)
...
unified_table %>%
  ...
  column_spec(4, bold = TRUE) # Emphasize the biological signal
```
