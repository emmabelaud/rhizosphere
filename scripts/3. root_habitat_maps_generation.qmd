...
Definition of buffer sizes: 236px=0.5cm, 500px=1cm, 945px=2cm

```{r}
buffer_distances <- c(236, 500, 945)
```
...
has_been_processed <- function(image_name) {
  ...
  processed_basenames <- unique(sub("_buffer_.*$", "", sub("_root_.*$", "", sub("\\.tif$", "", basename(existing_files)))))
...
```{r}
apply_buffer <- function(sf_object, buffer_dist) {
  message(paste("Applying buffer of", buffer_dist, "units..."))
  return(st_buffer(sf_object, dist = buffer_dist))
}
```
...
process_images <- function(past_image_file, current_image_file, area_threshold) {
  ...
  for (dist in buffer_distances) {
    buffer_name <- paste0("buffer_mature_", dist)
    habitat_variables[[buffer_name]] <- apply_buffer(habitat_variables$root_mature, dist)
    raster_name <- paste0("raster_", buffer_name)
    habitat_variables[[raster_name]] <- convert_to_raster(habitat_variables[[buffer_name]], root_mature)
    
    buffer_name_growth <- paste0("buffer_growth_", dist)
    habitat_variables[[buffer_name_growth]] <- apply_buffer(habitat_variables$root_growth, dist)
    raster_name_growth <- paste0("raster_", buffer_name_growth)
    habitat_variables[[raster_name_growth]] <- convert_to_raster(habitat_variables[[buffer_name_growth]], root_growth)
  }
...
for (scanner in unique(df_images$scanner)) {
  ...
  for (i in seq_len(nrow(df_scanner))) {
    ...
    for (dist in buffer_distances) {
      save_raster(processed[[paste0("raster_buffer_mature_", dist)]], current_basename, paste0("buffer_mature_", dist))
      save_raster(processed[[paste0("raster_buffer_growth_", dist)]], current_basename, paste0("buffer_growth_", dist))
    }
