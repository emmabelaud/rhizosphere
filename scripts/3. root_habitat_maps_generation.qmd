...
```{r}
dataset_dir <- "/media/ebelaud/INTENSO/rhizosphere_image_bank"
...
```{r}
output_raster_dir <- "/media/ebelaud/INTENSO/output_raster"
...
```{r}
for (scan in unique(df_images$scanner)) {
  df_scanner <- filter(df_images, scanner == scan)
