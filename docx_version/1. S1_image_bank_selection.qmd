---
title: "Supplementary Information S1 : Curated image bank"
format:
  docx:
    toc: false
...
#| fig-cap: "Presence/absence of images by scanner and date"
#| fig-width: 6
#| fig-height: 4.5
#| fig-align: "center"
#| fig-dpi: 300
df_complete <- df_complete %>%
  mutate(datetime = as.POSIXct(paste(day, .data$hour), format = "%Y-%m-%d %H:%M"),
...
We then identify the days on which at least **65% of these images are present** :
...
```{r}
#| label: tbl-selected-periods-summary
#| tbl-cap: "Summary of selected periods"
...
synthesis_table %>%
  ...
  style_ft() %>%
  width(j = c("period", "start_date", "final_date", "nb_images"), width = 0.9) %>%
  width(j = c("crops", "seasons"), width = 1.35)
...
cat("--- FINAL COMPLETENESS REPORT ---\n",
    "Initial target images (T0):      ", nrow(df_final), "\n",
      "Analyzable pairs (T0 + T-9):     ", nrow(df_final_filtered), "\n",
      "Number of missing images :", nrow(df_final)-nrow(df_final_filtered), "\n",
    "Missing frame rate :", round((1-(nrow(df_final_filtered)/nrow(df_final)))*100, 2), "%")
```
...
check_integrity(
  ...
  target = "/media/ebelaud/INTENSO/root_mask_data/", 
  mode = "folder", 
  file_ext = ".png"
)
...
check_integrity(
  expected_df = df_final_filtered, 
  target = "../output/distance_habitat_data.csv", 
  mode = "csv"
)

# 3. Checking the cleaned final data results
check_integrity(
  expected_df = df_final_filtered, 
