...
# Convenience wrapper — applies a consistent publication style to any flextable.
style_ft <- function(ft,
                     header_bg    = "white",
                     header_color = "black",
                     zebra_bg     = "white") {
  # Compute even-row indices directly (row_number() has no dplyr context in flextable)
  n_rows    <- nrow(ft$body$dataset)
  even_rows <- seq(2, n_rows, by = 2)
  ft %>%
    # Header background & bold white text
    bg(part = "header", bg = header_bg) %>%
    color(part = "header", color = header_color) %>%
    bold(part = "header") %>%
    # Alternating row shading — integer vector, not formula
    bg(i = even_rows, bg = zebra_bg, part = "body") %>%
    # Horizontal rules only (cleaner than full grid)
    hline(part = "body",
          border = fp_border_default(color = "#999999", width = 0.5)) %>%
    hline_bottom(part = "body",
                 border = fp_border_default(color = "black", width = 1.5)) %>%
    hline_top(part = "header",
              border = fp_border_default(color = "black", width = 1.5)) %>%
    # Vertical inner borders between columns (grey for body, navy for header)
    vline(border = fp_border_default(color = "#999999", width = 0.5), part = "body") %>%
    vline(border = fp_border_default(color = "black", width = 0.5), part = "header") %>%
    border_outer(border = fp_border_default(color = "black", width = 1)) %>%
    # Consistent cell padding
    padding(padding.top = 5, padding.bottom = 5,
            padding.left = 7, padding.right = 7, part = "all") %>%
    align(align = "center", part = "header") %>%
    align(j    = 1, align = "left",   part = "body") %>%
    # Prevent table rows from splitting across pages (cantSplit on <w:trPr>)
    # Remplacer la fin de la fonction style_ft par ceci :
...
taxa_summary %>%
  flextable() %>%
  style_ft() %>% # Applique ton template global
  
  # ── Remplacement de add_header_above() ──
  add_header_row(
    values = c("", "Biological Data", "Sampling Effort"),
    colwidths = c(1, 1, 2) # Taxon(1), Occurrences(1), Sample + Mean(2)
  ) %>%
  
  # ── Remplacement de footnote() ──
  add_footer_lines(
    values = c(
      paste("Total unique images analyzed: ", n_distinct(habitat_data_long$image_name)),
      paste("Total scanner units: ", n_distinct(habitat_data_long$scanner)),
      paste("Total period units: ", n_distinct(habitat_data_long$period))
    )
  ) %>%
  
  # Rendre le footer visuellement distinct (texte plus petit et gris)
  color(part = "footer", color = "#666666") %>%
  italic(part = "footer") %>%
  fontsize(part = "footer", size = 8) %>%
  
  # ── Remplacement de align = "lrrr" ──
  align(j = 1, align = "left", part = "all") %>%
  align(j = 2:4, align = "right", part = "body") %>%
  align(align = "center", part = "header") %>% # Centre les titres d'en-tête
  
  # Formatage propre des décimales
  colformat_double(j = "Mean occurrence per image", digits = 2) %>%
  
  # ── Ajustement des largeurs pour Word (optimisation de l'espace) ──
  width(j = "Taxon", width = 1.5) %>%
  width(j = c("Total occurrences", "Sample images"), width = 1.2) %>%
  width(j = "Mean occurrence per image", width = 1.6)
```
...
p1
tiff("../output/Figure.2.tiff", width = 1500, height = 1000, res = 250)
p1
invisible(dev.off())
```
...
model_perf %>%
  flextable() %>%
  style_ft() %>% # Ton template global
  
  # ── Remplacement de col.names ──
  set_header_labels(
    ...
    R_sq_adj           = "Adj. R²",
    Avg_EDF            = "Mean EDF",
    Significant_Terms  = "Significant Smooths",
    N_Total            = "N"
  ) %>%
  
  # ── Remplacement de digits = 3 ──
  colformat_double(
    j = c("AIC", "Deviance_Explained", "R_sq_adj", "Avg_EDF"), 
    digits = 3
  ) %>%
  
  # ── Remplacement de row_spec(2, bold = TRUE, background = "#FFF4E5") ──
  bold(i = 2, part = "body") %>%
  bg(i = 2, bg = "#FFF4E5", part = "body") %>%
  
  # ── Ajustement des largeurs pour éviter que le tableau déborde sur Word ──
  # (On cible toujours les colonnes par leur nom d'origine dans le dataframe)
  width(j = "Approach", width = 1.2) %>%
  width(j = c("AIC", "Deviance_Explained", "R_sq_adj", "Avg_EDF", "N_Total"), width = 0.8) %>%
  width(j = "Significant_Terms", width = 1.3) %>%
  
  # ── Remplacement de tbl-cap pour une lecture native dans Word ──
  set_caption(
...
s_tab_1 %>%
  ...
  # ── Remplacement de mutate(across(..., round)) ──
  colformat_double(
    j = c("Complexity (EDF)", "Ref.df", "Chi-square", "p-value"), 
    digits = 3
  ) %>%
  
  # ── Remplacement de align = "c" ──
  align(align = "center", part = "all") %>%
  
  # ── Remplacement de cell_spec() pour mettre en évidence la colonne Sign. ──
  color(j = "Sign.", color = "#D7261E", part = "body") %>%
  bold(j = "Sign.", bold = TRUE, part = "body") %>%
  
  # ── Remplacement de column_spec(7, background = "#f9f9f9") ──
  bg(j = "Sign.", bg = "#f9f9f9", part = "body") %>%
  
  # ── Remplacement de collapse_rows(columns = 1, valign = "top") ──
  merge_v(j = "Taxa") %>%
  valign(j = "Taxa", valign = "top", part = "body") %>%
  
  # ── Ajustement des largeurs pour Word ──
  width(j = "Taxa", width = 1.0) %>%
  width(j = "Root Type", width = 0.9) %>%
  width(j = c("Complexity (EDF)", "Ref.df", "Chi-square", "p-value"), width = 0.8) %>%
  width(j = "Sign.", width = 0.5) %>%
  
  # ── Légende native pour Word ──
  set_caption(
    caption = "Significance of smooth terms for root effects across taxa and root types.",
    style   = "Table Caption"
  )
```
...
get_model_diag_pro <- function(model, label, dharma_p) {
  ...
  data.frame(
    ...
    Dev_Expl   = paste0(round(sum_mod$dev.expl * 100, 1), "%"), 
...
diagnostic_report %>%
  ...
  # ── Remplacement de align = "lccccc" ──
  align(j = "Hypothesis", align = "left", part = "all") %>%
  align(j = c("Family", "Dev_Expl", "Dispersion", "DHARMa_p", "Status"), align = "center", part = "all") %>%
  
  # ── Remplacement de row_spec(0, bold = TRUE, background = "#F2F2F2") ──
  # (Même si style_ft le fait en partie, on force le fond gris ici selon ta demande)
  bg(bg = "#f2f2f2", part = "header") %>%
  bold(part = "header") %>%
  
  # ── Ajustement des largeurs pour Word ──
  width(j = c("Hypothesis", "Family"), width = 1.3) %>%
  width(j = c("Dev_Expl", "Dispersion", "DHARMa_p", "Status"), width = 1.0) %>%
  
  # ── Ajout de la légende Word ──
  set_caption(
    caption = "Diagnostic report for the density-driven and performance-based models.",
    style   = "Table Caption"
  )
```
...
unified_table %>%
  flextable() %>%
  style_ft() %>% # Ton template global (qui gère row_spec 0 background et les bordures)
  
  # ── Remplacement de add_header_above() et col.names() ──
  # On définit d'abord les étiquettes de la première ligne d'en-tête (les noms réels)
  set_header_labels(
    Hypothesis       = "Hypothesis",
    Family           = "Family",
    Total_D2         = "Total D²",
    Marginal_D2      = "Marginal D²",
    Random_effect_D2 = "Random D²",
    adj.r.squared    = "Adj. R²",
    AIC              = "AIC"
  ) %>%
  # On ajoute ensuite la ligne supérieure (sur-en-tête)
  add_header_row(
    values = c("", "Deviance Partitioning (D²)", "Fit Statistics"),
    colwidths = c(2, 3, 2) # Hypothesis+Family(2), D2(3), Stats(2)
  ) %>%
  
  # ── Remplacement de align = "lcccccc" ──
  align(j = "Hypothesis", align = "left", part = "all") %>%
  align(j = 2:7, align = "center", part = "all") %>%
  
  # ── Remplacement de mutate(across(..., round)) ou digits = 3 ──
  colformat_double(
    j = c("Total_D2", "Marginal_D2", "Random_effect_D2", "adj.r.squared", "AIC"),
    digits = 3
  ) %>%
  
  # ── Remplacement de cell_spec(bold = TRUE) ──
  bold(j = c("Hypothesis", "Marginal_D2"), bold = TRUE, part = "body") %>%
  
  # ── Remplacement de column_spec(4, background = "#f9f9f9") ──
  bg(j = "Marginal_D2", bg = "#f9f9f9", part = "body") %>%
  
  # ── Remplacement de column_spec(1, width = "150px") et sécurisation Word ──
  width(j = "Hypothesis", width = 1.6) %>% # ~150px
  width(j = "Family", width = 1.0) %>%
  width(j = c("Total_D2", "Marginal_D2", "Random_effect_D2", "adj.r.squared"), width = 0.8) %>%
  width(j = "AIC", width = 0.9) %>%
  
  # ── Ajout de la légende Word ──
  set_caption(
    caption = "Model comparison: Deviance partitioning and global fit statistics.",
    style   = "Table Caption"
  )
```
...
p3
tiff("../output/Figure.3.tiff", width = 2200, height = 1400, res = 300)
p3
invisible(dev.off())
```
